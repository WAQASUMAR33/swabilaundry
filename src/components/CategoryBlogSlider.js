...
export default function BlogCategorySlider({ category, blogs }) {
  ...
  const filteredBlogs = blogs.filter(blog => 
    blog.category.split(',').map(cat => cat.trim()).includes(category)
  );
  ...
  // Utility function to strip HTML tags and limit character count
            ...
            .map((blog, index) => (
              <div key={index} className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-gray-700 text-sm overflow-hidden line-clamp-2">
